...
-- SC is not sc per-location, thus X86 is not strictly stronger than mips.
-- The X86 is not strong enough to prove the atomicity
-- Because the X86 only make sure the execution is acyclic, but it's not.
